    ...
    if (error) {
      return (
        <View className="flex-1 items-center justify-center px-8 pt-20">
          ...
          <Text className="mt-1 text-center text-sm text-stone-400 dark:text-stone-500">{error}</Text>
          <Pressable
            onPress={refresh}
            className="mt-4 rounded-lg bg-indigo-600 px-5 py-2 active:bg-indigo-700 dark:bg-indigo-500 dark:active:bg-indigo-600"
          >
    ...
    if (debouncedSearch || activeFilterCount > 0) {
      return (
        <View className="flex-1 items-center justify-center px-8 pt-20">
          ...
          <Text className="mt-1 text-center text-sm text-stone-400 dark:text-stone-500">
      ...
      <View className="flex-1 items-center justify-center px-8 pt-20">
        ...
        <View className="mt-5 flex-row gap-3">
          <Pressable
            ...
            className="flex-row items-center rounded-lg bg-indigo-600 px-4 py-2.5 active:bg-indigo-700 dark:bg-indigo-500 dark:active:bg-indigo-600"
          >
          ...
          <Pressable
            ...
            className="flex-row items-center rounded-lg border border-stone-200 bg-white px-4 py-2.5 active:bg-stone-50 dark:border-stone-800 dark:bg-stone-900 dark:active:bg-stone-800"
          >
            <Ionicons name="download-outline" size={18} color={Colors.gray[600]} />
            <Text className="ml-1 text-sm font-medium text-stone-700 dark:text-stone-300">Import</Text>
          </Pressable>
        </View>
      </View>
    );
    ...
    <View className="flex-1 bg-stone-50 dark:bg-stone-950">
      ...
      <View className="flex-row items-center pr-4">
        ...
        <Pressable
          ...
          className="rounded-xl bg-white p-3 shadow-sm active:bg-stone-50 dark:bg-stone-900 dark:border dark:border-stone-800 dark:active:bg-stone-800"
        >
      ...
      {/* Action Row — icon-only buttons to save space */}
      <View className="flex-row items-center justify-end gap-2 px-4 pb-2">
        ...
        <Pressable
          onPress={() => router.push("/(app)/(tabs)/import")}
          className="rounded-full bg-white p-2 shadow-sm active:bg-stone-50 dark:bg-stone-900 dark:border dark:border-stone-800 dark:active:bg-stone-800"
          hitSlop={4}
        >
        ...
        <Pressable
          ...
          hitSlop={4}
        >
          <Ionicons name="link" size={16} color={Colors.brand[600]} />
        </Pressable>
      </View>
      ...
      {isLoading && contacts.length === 0 ? (
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-3 text-sm text-stone-400 dark:text-stone-500">Loading contacts...</Text>
        </View>
      ) : (
        <FlatList
          data={contacts}
          ...
          renderItem={({ item, index }) => (
            <Animated.View entering={FadeInDown.delay(Math.min(index, 10) * 50).duration(400).springify()}>
      ...
      <Pressable
        onPress={() => router.push("/contact/new")}
        className="absolute bottom-6 right-5 h-14 w-14 items-center justify-center rounded-full bg-indigo-600 shadow-lg active:bg-indigo-700 dark:bg-indigo-500 dark:active:bg-indigo-600"
        style={{
          elevation: 8,
