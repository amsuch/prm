    ...
    if (error) {
      return (
        <View className="flex-1 items-center justify-center px-8 pt-20">
          ...
          <Text className="mt-1 text-center text-sm text-gray-400">{error}</Text>
          <Pressable
            onPress={refresh}
      ...
      <View className="flex-1 items-center justify-center px-8 pt-20">
        ...
        <View className="mt-5 flex-row gap-3">
          ...
          <Pressable
            ...
            className="flex-row items-center rounded-lg border border-gray-200 bg-white px-4 py-2.5 active:bg-gray-50"
          >
            <Ionicons name="download-outline" size={18} color={Colors.gray[600]} />
            <Text className="ml-1 text-sm font-medium text-gray-700">Import</Text>
          </Pressable>
        </View>
      </View>
    );
    ...
    <View className="flex-1 bg-gray-50">
      ...
      {isLoading && contacts.length === 0 ? (
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-3 text-sm text-gray-400">Loading contacts...</Text>
        </View>
      ) : (
        <FlatList
          data={contacts}
      ...
      <Pressable
        onPress={() => router.push("/contact/new")}
        className="absolute bottom-6 right-5 h-14 w-14 items-center justify-center rounded-full bg-blue-600 shadow-lg active:bg-blue-700"
        style={{
          elevation: 8,
