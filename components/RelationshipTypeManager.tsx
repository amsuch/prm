    ...
    <View className="flex-row items-center justify-between px-4 py-3">
      <View className="flex-1">
        <View className="flex-row items-center">
          <Text className="text-sm font-medium text-gray-900">{type.name}</Text>
          {isAsymmetric && (
        ...
        {type.is_system && (
          <Text className="mt-0.5 text-xs text-gray-400">System</Text>
        )}
      </View>
      {!type.is_system && (
        <Pressable
    ...
    <View>
      ...
      {grouped.map((group, groupIndex) => (
        <View key={group.category}>
          {groupIndex > 0 && <View className="mx-4 h-px bg-gray-100" />}
          ...
          {group.types.map((type, index) => (
            <View key={type.id}>
              {index > 0 && <View className="ml-4 h-px bg-gray-50" />}
      ...
      {isAdding ? (
        <View className="border-t border-gray-100 px-4 py-3">
            ...
            {CATEGORIES.map((cat) => (
              <Pressable
                ...
                className={`rounded-lg px-3 py-1.5 ${
                  newCategory === cat ? "bg-blue-600" : "bg-gray-100"
                }`}
              >
                <Text
                  className={`text-xs font-medium ${
                    newCategory === cat ? "text-white" : "text-gray-600"
                  }`}
                >
                  {cat}
                </Text>
          ...
          <View className="mb-3 flex-row items-center justify-between">
            <View>
              <Text className="text-sm font-medium text-gray-700">Symmetric</Text>
