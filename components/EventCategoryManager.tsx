...
const ICON_OPTIONS: { name: string; label: string }[] = [
  ...
  { name: "business-outline", label: "Conference" },
  { name: "people-outline", label: "Meetup" },
  { name: "wine-outline", label: "Party" },
  { name: "construct-outline", label: "Workshop" },
  { name: "heart-outline", label: "Wedding" },
  { name: "musical-notes-outline", label: "Concert" },
  { name: "football-outline", label: "Sports" },
  { name: "restaurant-outline", label: "Dinner" },
  { name: "airplane-outline", label: "Travel" },
  { name: "school-outline", label: "Class" },
  { name: "trophy-outline", label: "Award" },
  ...
  if (isEditing) {
    return (
      <View className="px-4 py-3">
        ...
        {editError && (
          <Text className="mb-2 text-xs text-red-500 dark:text-red-400">
            {editError}
          </Text>
        )}
  ...
  const handleDelete = useCallback(
    (categoryId: string, categoryName: string) => {
      ...
      if (Platform.OS === "web") {
        if (
          window.confirm(
            `Delete "${categoryName}"? Events using this category will keep their current category text.`,
          )
        ) {
          doDelete();
        }
  ...
  return (
    <View>
      ...
      {categories.map((cat, index) => (
        <View key={cat.id}>
          {index > 0 && (
            <View className="ml-4 h-px bg-stone-100 dark:bg-stone-800" />
          )}
          <CategoryRow
            category={cat}
